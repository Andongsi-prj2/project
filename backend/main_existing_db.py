...
async def get_factory_dashboard():
    """공장 대시보드 데이터 - DATA_MART에서 CROM 데이터 조회"""
    try:
        with engine.connect() as conn:
            # DATA_MART 데이터베이스 사용
            conn.execute(text("USE DATA_MART"))
            
            # CROM 테이블에서 최근 데이터 조회
            query = text("""
                SELECT * FROM CROM 
                ORDER BY id DESC 
                LIMIT 50
            ...
            return {
                "status": "success",
                "message": "Factory dashboard data loaded from DATA_MART",
                "data": data,
                "total_count": len(data),
                "source": "DATA_MART.CROM",
                "last_updated": datetime.now().isoformat()
            }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
